...
function Order() {
  const { user, userId, loading: authLoading, setUser } = useContext(AuthContext)
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      if (setUser) {
        setUser(prev => (prev ? { ...prev, order: updatedOrders } : prev))
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchOrders()
  }, [userId, authLoading, fetchOrders, navigate])

  const handleCancelOrder = async (e, orderId) => {
    e.stopPropagation()
    const targetUserId = userId || user?.id || user?._id
    if (!targetUserId) { toast.error("User not found"); return }
    const confirm = window.confirm("Cancel this order?")
    if (!confirm) return
    try {
      setLoadingId(orderId)
      await cancelOrder(targetUserId, orderId)
      await fetchOrders()
  ...
  const goToProduct = (productId, status) => {
    if (status === "Cancelled" || !productId) return
          ...
          {!loading && orders.length > 0 && (
            <div className="space-y-6">
              {orders.map(order => {
                ...
                return (
                  <div key={orderId} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-all duration-300">
                      ...
                      {order.items?.map((item, idx) => (
                        ...
                        >
                          ...
                          <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl flex items-center justify-center overflow-hidden">
                            <img src={getImageUrl(item.img)} alt={item.name} className="w-full h-full object-contain p-2" />
                          </div>
